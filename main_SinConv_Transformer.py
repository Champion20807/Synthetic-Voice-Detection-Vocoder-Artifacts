...
from model_SinConv_Transformer import RawNetWithSincConvTransformer
...
SAMPLE_RATE = 16000  # Update to match the SincConv sample rate
...
if __name__ == '__main__':
    ...
    d_args = {
        'hidden_size': 256,  # Ensure this matches SincConv output dimensions
        'num_heads': 4,
        'ffn_dim': 512,
        'dropout': 0.1,
        'num_layers': 2,
    ...
    dev_dataloader = DataLoader(dev_set, batch_size=batch_size, shuffle=False, drop_last=False)
    ...
    best_acc = 0  # Initial best accuracy
    for epoch in range(starting_epoch_idx + 1, num_epochs):
        ...
        print(out_write)
        print(f'Epoch: {epoch} - Loss: {running_loss:.4f} - Valid Accuracy: {valid_accuracy:.2f}')

        # Save model for best accuracy
        if valid_accuracy > best_acc:
            best_acc = valid_accuracy
            best_model_path = os.path.join(model_save_path, 'best_acc_epoch.pth')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        epoch_model_path = os.path.join(model_save_path, f'epoch_{epoch}.pth')
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
